...
-(BOOL)isEqual:(id)object{
    if ([object isKindOfClass:[GLDedNode class]]) {
        ...
        if (_rule!=otherNode.inferenceRule) return FALSE;
        if (![_deps isEqualToArray:otherNode.dependencies]) return FALSE;
        if (![_infs isEqualToArray:otherNode.inferenceNodes]) return FALSE;
        return TRUE;
    }else return FALSE;
}
...
/**
 *  Returns a GLDedNode with the parameter form formula property, inf inference property and nodes inference nodes. The return node also inherits dependencies from the parameter node.<p/>
